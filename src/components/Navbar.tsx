...
const iconPaths = [
  { path: "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z", href: ROUTES.SEARCH.path, label: 'Search' },
  { path: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z", href: ROUTES.ACCOUNT.path, label: 'Account' },
  { path: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z", href: ROUTES.WISHLIST.path, label: 'Wishlist' },
  { path: "M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z", href: ROUTES.CART.path, label: 'Cart' },
];

const IconButton = ({ children, href, label }: { children: React.ReactNode; href: string; label: string }) => {
      ...
      <nav className="hidden md:block bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <Link href="/" className="text-2xl font-bold hover:opacity-80 transition-opacity duration-200">
              ...
              {categories.map((category) => (
                <Link
                  ...
                  className={`text-gray-700 hover:text-gray-900 transition-colors duration-200 relative after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-blue-600 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300 ${
                    pathname === category.href ? 'text-blue-600 after:scale-x-100' : ''
                  }`}
                >
                  {category.name}
                </Link>
